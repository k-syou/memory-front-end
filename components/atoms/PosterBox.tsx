  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(PosterBoxVariants({ contentSize }), "py-4 pl-[18px]")}
        >
          <Text
            type="paragraph"
            className={`text-b1 ${isHover ? hoverTextColor : "text-black"}`}
          >
            {title}
          </Text>
          <Text
            type="paragraph"
            className="text-b1 text-gray500"
          >{`${startYY_MM} ~ ${endYY_MM}`}</Text>
        </div>
      </div>
    );
  }
